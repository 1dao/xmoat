...
**谁现在在发信号？**
- 「横盘之后上穿均线」这类价格规则，可以在一组股票上扫描当前信号；规则的数字是配置项（`BREAKOUT_*`），网格用来选它们
...
网页上的「筛选」页分两种：**定制规则**是自己设条件（ROE、估值、增速……）在快照里筛；
**内置规则**是引擎自带、数字由配置决定的规则，目前是「横盘之后上穿均线」（默认横盘 8 周、
振幅 ≤10%，当天收盘上穿 10 周均线），点「筛选全市场」就在全部 A 股里找刚刚**首次突破**的股票——
...
```bash
bin/xnet.exe cli.lua scan universe=market limit=6000 ma_weeks=8 above_pct=3
```
...
- [x] **Phase 7 · 规则与批量**
  - [x] 横盘后上穿均线：参数网格拟合（单只 / 一组股票）、当前信号扫描
  - [x] 网页「筛选 → 内置规则」：突破横盘周线，一键扫全市场
  - [x] 信号记录：首日突破记下加入时间与价格，跟踪之后的涨幅；收盘后自动跑，只推送新加入的
...
- **通达信的日线没有换手率**，所以用它抓的股票没有筹码分布。自选股和指数先走东方财富，
  换手率和筹码都在；东方财富拒绝时（它会整段时间封一个地址）改用通达信，当天没有筹码，
  它恢复后整段重取回来。通达信那条路主要是给几百只股票的扫描用的。
